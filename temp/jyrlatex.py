# -*- coding: utf8 -*-
"""
A Printer which converts an expression into its LaTeX equivalent with colored braces.
-- slightly modified from original latex module
"""
...
from jyrsympify import SympifyError
...
class LatexPrinter(Printer):
    ...
    def parenthesize(self, item, level):
        if precedence(item) <= level:
            ...
            term_tex = color[colr](r'\left(%s\right)' % self._print(item))
            return term_tex
        else:
    ...
    def _print_Mul(self, expr):
        ...
        if not coeff.is_negative:
            tex = ""
            need_brackets = 0;
        elif (not coeff == -1) or (not (-expr).func == Add):
            expr = -expr
            tex = "- "
            need_brackets = 0;
        else:
        ...
        def convert(expr, need_brackets=0):
            if (not expr.is_Mul) and (need_brackets == 0):
        ...
        if denom is S.One:
            ...
            # fixed bug when coefficient is -1
