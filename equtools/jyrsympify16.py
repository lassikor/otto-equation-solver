...
def _convert_numpy_types(a, **sympify_args):
    ...
    if not isinstance(a, np.floating):
        if np.iscomplex(a):
            ...
            return jyrsympify(a.item(), **sympify_args)
    else:
...
def jyrsympify(a, locals=None, convert_xor=True, strict=False, rational=False,
        evaluate=None):
    """
    ...
    if not isinstance(a, str):
        if _is_numpy_instance(a):
            ...
            return jyrsympify(float(a))
        elif hasattr(a, '__int__'):
            return jyrsympify(int(a))
    ...
    if iterable(a):
        try:
            return type(a)([jyrsympify(x, locals=locals, convert_xor=convert_xor,
...
def _sympify(a):
    ...
    """
    return jyrsympify(a, strict=True)
    ...
    for i in range(2):
        try:
            ...
            if hit:
                ...
                hit = False
                continue
            expr = jyrsympify(s)  # let original error raise
