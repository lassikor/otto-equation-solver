"""otsympify -- convert objects SymPy internal format, a slightly modified version of original sympify"""
...
def _convert_numpy_types(a, **sympify_args):
    ...
    if not isinstance(a, np.floating):
        if np.iscomplex(a):
            ...
            return otsympify(a.item(), **sympify_args)
    else:
...
def otsympify(a, locals=None, convert_xor=True, strict=False, rational=False,
        evaluate=None):
    """
    ...
    if not isinstance(a, str):
        if _is_numpy_instance(a):
            ...
            return otsympify(float(a))
        elif hasattr(a, '__int__'):
            return otsympify(int(a))
    ...
    if iterable(a):
        try:
            return type(a)([otsympify(x, locals=locals, convert_xor=convert_xor,
    ...
    try:
        ...
        expr.color = ['black'] #attach color to expr
...
def _sympify(a):
    ...
    """
    return otsympify(a, strict=True)
    ...
    for i in range(2):
        try:
            ...
            if hit:
                ...
                hit = False
                continue
            expr = otsympify(s)  # let original error raise
