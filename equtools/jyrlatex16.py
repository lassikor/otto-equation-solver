...
color = {
    #colors for braces
    'green': lambda s: r'{\color{green}'+s+r'}',
    'red': lambda s: r'{\color{red}'+s+r'}',
    'blue': lambda s: r'{\color{blue}'+s+r'}',
    'magenta': lambda s: r'{\color{magenta}'+s+r'}'
}

#if colorlist is edited, please update the rem_colors list too
colorlist_fi = [('black','musta'),
                ('red','punainen'),
                ('blue','sininen'),
                ('green','vihreä'),
                ('magenta','violetti')]
...
class LatexPrinter(Printer):
    ...
    def __init__(self, settings=None):
        ...
        self.rem_colors = ['red','blue','green','magenta']
    ...
    def parenthesize(self, item, level, is_neg=False, strict=False):
        ...
        if (prec_val < level) or ((not strict) and prec_val <= level):
            colr = self.rem_colors.pop(0)
            term_tex = color[colr](r"\left(%s\right)".format(self._print(item)))
            return term_tex
        else:
    ...
    def _print_Mul(self, expr):
        ...
        def convert(expr, pass_coloring = False):
            if not expr.is_Mul and (include_parens == 0):
                ...
                for i, term in enumerate(args):
                    ...
                    if self._needs_mul_brackets(term, first=(i == 0),
                                                last=(i == len(args) - 1)):
                        if not pass_coloring:
                            colr = self.rem_colors.pop(0)
                            if not hasattr(term,'color') or term.color == []:
                                term.color = [colr]
                           
                            elif term.color.count(colr) == 0:
                                term.color.append(colr)
                                
                            term_tex = color[colr](r'\left(%s\right)' % term_tex)
        ...
        if denom is S.One and Pow(1, -1, evaluate=False) not in expr.args:
            ...
            if self._settings['fold_short_frac'] and ldenom <= 2 and \
                ...
                if self._needs_mul_brackets(numer, last=True):
                    if hasattr(numer,'color'):
                        colr = numer.color[-1]
                    elif not hasattr(numer,'color') or numer.color == []:
                        colr = self.rem_colors.pop(0)
                        numer.color = [colr]
                   # elif numer.color.count(colr) == 0:
                   #     numer.color.append(colr)
                    tex += r"\frac{1}{%s}%s" % (sdenom, separator)
                    tex += color[colr](r'\left(%s\right)' % snumer)
                elif numer.is_Mul:
                    ...
                    for x in numer.args:
                        if self._needs_mul_brackets(x, last=False) or \
                                len(convert(a*x, pass_coloring = True).split()) > ratio*ldenom or \
                                (b.is_commutative is x.is_commutative is False):
                            b *= x
                        else:
                            a *= x
                    if self._needs_mul_brackets(b, last=True):
                        if hasattr(b,'color') and not b.color == []:
                            colr = b.color[-1]
                        elif not hasattr(b,'color') or b.color == []:
                            colr = self.rem_colors.pop(0)
                            b.color = [colr]
                       # elif b.color.count(colr) == 0:
                       #     b.color.append(colr)
                               
                        tex += r"\frac{%s}{%s}%s" % (convert(a), sdenom, separator)
                        tex += color[colr](r'\left(%s\right)' % convert(b))
                    else:
                        tex += r"\frac{%s}{%s}%s%s" \
